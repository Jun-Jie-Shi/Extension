...
def notemp_kl_loss_bs(pred_s, pred_t, target, num_cls=5, temp=1.0, up_op=None):

    pred_s = torch.clamp(pred_s, min=0.005, max=1)
    pred_t = torch.clamp(pred_t, min=0.005, max=1)
    pred_s = torch.log(pred_s)
    kl_loss = torch.mul(pred_t, torch.log(pred_t)-pred_s)
...
def prototype_passion_loss_bs(feature_s, feature_t, target, logit_s=None, logit_t=None, num_cls=5, temp=1.0, up_op=None):
    ...
    # dce_loss = torch.mean(-torch.log(torch.clamp(sim_map_s_gt, min=0.005, max=1)), dim=(1,2,3)).unsqueeze(1)
    # pl_loss = torch.mean(dist_map_s_gt, dim=(1,2,3)).unsqueeze(1)
    # proto_loss = dce_loss + 0.001 * pl_loss

    # dist = torch.mean(sim_map_s_gt, dim=(1,2,3)).unsqueeze(1)
